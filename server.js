...
const EXPIRATION_TIME_MS = 12 * 60 * 60 * 1000; // 12 horas em milissegundos
const VIP_SECTION_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutos para páginas VIP específicas
...
app.use(session({
  ...
  cookie: {
    ...
    // Se process.env.NODE_ENV === 'production' for VERDADEIRO, o cookie 'secure' será true.
    // Isso significa que o cookie SÓ será enviado pelo navegador em conexões HTTPS.
    // Se você estiver testando em HTTP (ex: http://localhost:3000) e NODE_ENV for 'production',
    // o cookie não será enviado, a sessão não será reconhecida na próxima página,
    // e o middleware 'isAuthenticated' irá falhar, redirecionando para o login.
    // SOLUÇÃO: Garanta que NODE_ENV NÃO seja 'production' ao testar em HTTP,
    // ou configure seu ambiente de desenvolvimento para usar HTTPS.
    secure: process.env.NODE_ENV === 'production', //
    httpOnly: true, //
    maxAge: 24 * 60 * 60 * 1000 // Cookie da sessão dura 24 horas
  }
}));
...
app.post('/api/login', (req, res) => { //
  ...
  if (tipoLogado && credencialEncontrada) { //
    ...
    delete req.session.vipSectionEntryTimestamp; // Limpa o timer VIP em novo login
